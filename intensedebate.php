...
// CONSTANTS
	...
	define( 'ID_PLUGIN_VERSION', '2.10.1' );
...
// HOOK ASSIGNMENT
	function id_activate_hooks() {
		...
		if ( id_do_admin_hooks() ) {
			...
			add_filter( 'allowed_options', 'id_allowed_options' );
		...
		if ( id_is_active() ) {
			...
			add_filter( 'option_moderation_notify', '__return_zero' );
			add_filter( 'option_comments_notify', '__return_zero' );
		}
	...
	function id_allowed_options( $options ) {
...
// UTILITIES
	...
	// escapes a string for html output in an attribute
	function id_esc_attr( $str ) {
...
// DATA WRAPPERS
	...
	class id_data_wrapper {
		...
		public function __construct( $props = null, $bRemoteLabels = false ) {
			if ( isset( $props ) ) {
				if ( $bRemoteLabels ) {
...
// COMMENT WRAPPER
	...
	class id_comment extends id_data_wrapper {
		...
		public function __construct( $props = null, $bRemoteLabels = false ) {
			...
			parent::__construct( $props, $bRemoteLabels );
		}
...
// QUEUE
	...
	class id_queue_operation {
		...
		public function __construct( $action, $data, $callback = null ) {
			$this->action = $action;
...
// ACTION: import
	...
	function id_REST_import() {
		...
		$total_comment_count_query = "SELECT COUNT(comment_ID) FROM {$wpdb->comments} WHERE$post_where comment_approved != 'spam'";
		$result->totalCommentCount = $wpdb->get_var(
			empty( $tot_params )
				? $total_comment_count_query
				: $wpdb->prepare( $total_comment_count_query, $tot_params )
		);
...
// Enter a new comment in to the system
	...
	function id_REST_save_comment() {
		...
		if ( property_exists( $comment, 'comment_status' ) && 'delete' == $comment->comment_status ) {
	...
	**/
	function id_discussion_settings_page( $merge_moderation_strings = false ) {
		...
		if ( ( isset( $_POST['option_page'] ) && 'discussion' == $_POST['option_page'] ) || ( isset( $_POST['page_options'] ) && stristr( $_POST['page_options'], 'comment_moderation' ) ) ) {
			...
			$sync = false;
			$options = array( 'comments_notify', 'moderation_notify', 'comment_moderation', 'comment_previously_approved', 'comment_max_links', 'moderation_keys', 'disallowed_keys', 'thread_comments' );
			...
			if ( $sync ) {
				...
				$settings = array(
									...
									'require_previously_approved'   => ( '1' == $_POST['comment_previously_approved']  ? 'T' : 'F' ),
				...
				$blacklist = id_separate_tokens( $_POST['disallowed_keys'] );
	...
	**/
	function id_discussion_sync_from_db( $merge_moderation_strings = false ) {
		$settings = array(
							...
							'require_previously_approved'   => ( '1' == get_option( 'comment_previously_approved' )  ? 'T' : 'F' ),
		...
		$blacklist = id_separate_tokens( get_option( 'disallowed_keys' ) );
		$settings['blacklisted_words']  = implode( ' ', $blacklist['words'] );
		$settings['blacklisted_ips']    = implode( ' ', $blacklist['ips'] );
		$settings['blacklisted_emails'] = implode( ' ', $blacklist['emails'] );
...
// SETTINGS PAGE
						...
						<?php if ( get_option( 'id_signup_step' ) == 0 ) : // first step (login/signup) ?>
							...
							<form id="id_user_login" action="options-general.php?page=id_settings" method="POST">
								...
								<div class="idwp-install-form_elements form-table">
									...
									<div class="idwp-input-text-wrap">
								    	<input id="txtEmail" autocomplete="off" type="text" class="required regular-text" name="id_remote_fields[username]" value="<?php echo id_esc_attr( $username ); ?>" />
									</div>
							...
							<tbody>
								<tr valign="top">
									...
									<td>
										...
										<span class="idwp-clear"></span>
										<p id="divCommentSystemInfo" class="hidden description"><?php _e( "If you select to use the WordPress comment template, then we won't load the IntenseDebate comment system on your blog.", 'intensedebate' ); ?></p>
									</td>
								</tr>
								<tr valign="top">
									...
									<td>
										...
										<span class="idwp-clear"></span>
										<p id="divCommentLinkInfo" class="hidden description"><?php printf( __( 'Use customized comment link text by enabling IntenseDebate Enhanced Comment Links.  <a href="%s">Learn more</a> about customizing your comment links.', 'intensedebate' ), ID_BASEURL . '/faq#li181' ); ?></p>
									</td>
								</tr>
								<tr valign="top">
									...
									<td>
										...
										<span class="idwp-clear"></span>
										<p id="divRevertMobileInfo" class="hidden description"><?php _e( 'This setting will determine if we show IntenseDebate comments or WordPress comments when a reader on a mobile device visits your blog.  Because IntenseDebate is not yet fully compatible with all mobile devices, we suggest reverting to the standard WordPress comments when mobile devices access your blog.', 'intensedebate' ); ?></p>
									</td>
								</tr>
								<tr valign="top">
	...
	function id_message() {
		if ( $msg = id_param( 'msg' ) ) {
			?>
			<div id="message" class="updated fade"><p><strong><?php echo id_esc_html( $msg ); ?></strong></p></div>
			<?php
		}
	}
...
// COMMENT MODERATION PAGE
	...
	function id_moderate_comments() {
		...
		<div class="wrap">
			...
			<iframe frameborder="0" id="id_iframe_moderation" src="<?php echo $iframe_url . get_option( 'id_blogID' ); ?>" style="width: 100%; height: 500px; border: none;" onload="addScript()" scrolling="auto" sandbox="allow-same-origin allow-scripts allow-forms"></iframe>
		</div>
